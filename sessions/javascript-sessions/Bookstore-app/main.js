...
function showData(){
    ...
    bookList2.forEach(function (value2, i) {
        ...
        <tr>
            ...
            <td>${value2.year}</td>
            <td><button class="btn btn-sm btn-warning" 
                    onclick="unread(${value2.id}, '${value2.title}', '${value2.author}', ${value2.year})">
                    <i class="fa fa-check"></i>
                </button>
            </td>
            <td><button class="btn btn-sm btn-success" 
                    onclick="findRead(${value2.id})">
                    <i class="fa fa-edit"></i>
                </button>
            </td>
            <td><button class="btn btn-sm btn-danger" 
                onclick="removeRead(${value2.id})">
                     <i class="fa fa-trash"></i>
                </button>
            </td>
        </tr>`;
...
function unread(id1, title1, author1, year1) {
    if (id1) {
        let item = [{
            id: id1,
            title: title1,
        ...
        books = item.concat(bookList);
        localStorage.setItem('listItem4', JSON.stringify(books));
    }

    let bookList3 = JSON.parse(localStorage.getItem('listItem3')) ?? [];
    bookList3 = bookList3.filter(value => value.id != id1);
    localStorage.setItem('listItem3', JSON.stringify(bookList3));
    showData();
}
function findRead(id1) {
    ...
    bookList.forEach(value => {
        if (value.id == id1) {
            ...
            document.getElementById('bookAuthor').value = value.author;
            document.getElementById('bookYear').value = value.year;
            document.getElementById('bookComplete').checked = true;
        }
    });
}
function removeRead(id1) {
    let bookList = JSON.parse(localStorage.getItem('listItem3')) ?? [];
    bookList = bookList.filter(value => value.id != id1);
    localStorage.setItem('listItem3', JSON.stringify(bookList));
    showData();
}
